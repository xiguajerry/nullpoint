...
public class EntityUtil implements Wrapper {
   ...
   public static boolean isInsideBlock() {
      return BlockUtil.getBlock(getPlayerPos(true)) == Blocks.ENDER_CHEST || mc.world.canCollide(mc.player, mc.player.getBoundingBox());
   }
   ...
   public static boolean isArmorLow(PlayerEntity player, int durability) {
      ...
      } while(getDamagePercent(piece) >= durability);
   ...
   public static boolean canSee(BlockPos pos, Direction side) {
      ...
      HitResult result = mc.world.raycast(new RaycastContext(getEyesPos(), testVec, ShapeType.COLLIDER, FluidHandling.NONE, mc.player));
   ...
   public static void faceVector(Vec3d directionVec) {
      ...
      if (angle[0] != Nullpoint.ROTATE.lastYaw || angle[1] != Nullpoint.ROTATE.lastPitch) {
   ...
   public static void sendLook(PlayerMoveC2SPacket packet) {
      if (packet.changesLook() && (packet.getYaw(114514.0F) != Nullpoint.ROTATE.lastYaw || packet.getPitch(114514.0F) != Nullpoint.ROTATE.lastPitch)) {
         rotating = true;
         Nullpoint.ROTATE.setRotation(packet.getYaw(0.0F), packet.getPitch(0.0F), true);
